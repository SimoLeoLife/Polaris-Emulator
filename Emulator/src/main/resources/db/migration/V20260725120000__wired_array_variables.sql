...
-- Wired furniture rows. Fixed sprite IDs match FurnitureData; database row IDs remain
-- installation-local. A hotel that already carries the furni (from its own furnidata) keeps
-- its row and only has interaction_type realigned, the same shape as the other wired
-- furniture migrations, so re-running this migration is a no-op rather than a failure.
--
-- The 0/1 flags are written as strings on purpose. items_base ships here as tinyint(1), but
-- Arcturus-derived hotels carry enum('0','1') for the same columns, where an integer means the
-- Nth member (1-based) - so an unquoted 0 is the invalid enum value and the insert dies with
-- "Data truncated for column 'allow_sit'". A quoted '0'/'1' is read as the literal value by
-- both column types.
...
-- Point an already-present row at its registered interaction, so a hotel that installed the
-- furni itself gets a working box instead of an inert 'default' one.
UPDATE `items_base` SET `interaction_type` = 'wf_act_modify_array'
    WHERE `item_name` = 'wf_act_modify_array' AND `interaction_type` <> 'wf_act_modify_array';

UPDATE `items_base` SET `interaction_type` = 'wf_cnd_check_array'
    WHERE `item_name` = 'wf_cnd_check_array' AND `interaction_type` <> 'wf_cnd_check_array';

UPDATE `items_base` SET `interaction_type` = 'wf_xtra_array_capture_variable'
    WHERE `item_name` = 'wf_xtra_array_capture_variable'
      AND `interaction_type` <> 'wf_xtra_array_capture_variable';
